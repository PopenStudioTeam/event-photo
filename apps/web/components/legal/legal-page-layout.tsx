    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="animate-in fade-in slide-in-from-bottom-2 rounded-xl border border-amber-500/30 bg-amber-500/10 px-5 py-4 text-sm leading-6 text-foreground duration-500">
          ...
          a structural starting point, not legal or security advice. Replace
          every bracketed placeholder with your actual company details and
          practices, and have it reviewed before publishing.
        </div>
        ...
        {intro && intro.length > 0 && (
          <div className="mt-8 space-y-4 text-sm leading-7 text-foreground/85">
            {intro.map((paragraph, index) => (
              <p key={index}>{renderLegalText(paragraph)}</p>
            ))}
          </div>
        )}
            ...
            <ul className="mt-3 space-y-2 text-sm leading-6 text-muted-foreground">
              {keyPoints.map((point, index) => (
                <li key={index} className="flex gap-2">
                  ...
                  <span>{renderLegalText(point)}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="mt-12 space-y-12">
          {sections.map((section, index) => (
            <div key={section.heading}>
              <h2
                className={
                  !numbered && section.subsections
                    ? "text-2xl font-bold text-foreground"
                    : "text-xl font-bold text-foreground"
                }
              >
                {numbered ? `${index + 1}. ${section.heading}` : section.heading}
              </h2>

              {section.paragraphs && (
                <div className="mt-3 space-y-3 text-sm leading-7 text-foreground/85">
                  {section.paragraphs.map((paragraph, pIndex) => (
                    <p key={pIndex}>{renderLegalText(paragraph)}</p>
                  ))}
                </div>
              )}
